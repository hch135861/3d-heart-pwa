/** Code410 boundary adapter. Invalid or contradictory data never becomes a zero ETA. */
...
export function parseDuration(value) {
  ...
  if (/^(?:即将到站|即将进站|进站中|已到站|到站)$/.test(s)) return { kind: 'valid', seconds: 0, label: '即将到站' };
  let m = s.match(/^(\d+(?:\.\d+)?)(?:-|~|至|–)(\d+(?:\.\d+)?)(分钟|分|秒)$/);
  if (m) {
    const lo = Number(m[1]), hi = Number(m[2]), unit = m[3] === '秒' ? 1 : 60;
    return lo <= hi && hi * unit <= MAX_SECONDS
      ? { kind: 'valid', seconds: hi * unit, label: `${lo}–${hi}${unit === 60 ? '分钟' : '秒'}` }
...
function dataQuality({ stationVerified, stops, duration, arrival, issues }) {
  if (issues.length) return { score: 0, label: '低', note: '存在异常字段' };
  let score = stationVerified ? 2 : 0;
  if (stops !== null) score += 1;
  if (duration.kind === 'valid') score += 1;
  if (arrival.kind === 'valid') score += 1;
  const label = score >= 5 ? '高' : score >= 3 ? '中' : '低';
  const note = label === '高' ? '站点、站数与两类时间相互一致'
    : label === '中' ? '核心时间可用，但部分辅助字段缺失'
    : '可用字段较少';
...
export function normalizeRow(raw, { now, side, sourceKey, stationVerified }) {
  ...
  const line = cleanLabel(raw.line, '未标明线路');
  const destination = cleanLabel(raw.destination, '未标明终点');
  ...
  const vehicleId = cleanLabel(raw.licence_plate || raw.license_plate || raw.vehicle_id || raw.bus_id);
  const quality = dataQuality({ stationVerified, stops, duration, arrival, issues });
  const timeSource = duration.kind === 'valid' && arrival.kind === 'valid' ? '等待时长+到站时刻'
    : duration.kind === 'valid' ? '等待时长' : arrival.kind === 'valid' ? '到站时刻' : '无有效时间';

  return {
    line, destination, stops, seconds, eta, toMinhui, issues, quality, timeSource, vehicleId,
    arrivalMs: issues.length ? null : arrival.ms, side, sourceKey, receivedAt: now, stationVerified,
    key: JSON.stringify([side, sourceKey, line, destination, vehicleId, text(raw.estimated_time), text(raw.expected_arrival), stops])
  };
}
...
export function buildCommutePlan(view, walkMinutes = 4, bufferMinutes = 1) {
  const walk = Math.max(0, Math.min(30, Number(walkMinutes) || 0));
  const buffer = Math.max(0, Math.min(10, Number(bufferMinutes) || 0));
  const candidates = view.candidates.filter(r => r.toMinhui).sort((a, b) => a.seconds - b.seconds);
  if (!candidates.length) return { kind: 'unknown', title: '暂时无法判断何时出门', detail: '没有可信的民辉方向到站预估。', target: null, first: null };
  const first = candidates[0];
  const need = (walk + buffer) * 60;
  const catchable = candidates.find(r => r.seconds >= need) || null;

  if (first.seconds < need) {
    if (catchable) return {
      kind: 'next', title: `别追 ${first.line}，准备等 ${catchable.line}`,
      detail: `按步行 ${walk} 分钟 + ${buffer} 分钟余量，首班可能赶不上；现在出门更适合等下一班。`,
      target: catchable, first
    };
    return {
      kind: 'tight', title: `${first.line} 可能赶不上`,
      detail: `按步行 ${walk} 分钟 + ${buffer} 分钟余量，目前没有更晚且可信的班次可比较。`,
      target: null, first
    };
  }

  const spare = first.seconds - need;
  if (spare <= 90) return {
    kind: 'go', title: `现在出门 · 等 ${first.line}`,
    detail: `按步行 ${walk} 分钟 + ${buffer} 分钟余量，时间比较合适。`,
    target: first, first
  };

  const waitMinutes = Math.max(1, Math.floor(spare / 60));
  return {
    kind: 'wait', title: `可以再等约 ${waitMinutes} 分钟`,
    detail: `按当前预估，之后出门仍可留出步行 ${walk} 分钟 + ${buffer} 分钟余量；刷新后再确认。`,
    target: first, first, waitMinutes
  };
}
